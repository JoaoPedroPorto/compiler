...
public class Lexicon {
	...
	public Token nextToken() throws IOException {
		...
		switch (character) {
			...
			default:
				...
				lexeme.append(character);
				errors.addErro("Caracter invalido: ", lexeme.toString(), line, column, "LEX");
				this.nextToken();
		}
		return token;
	}
	...
	private Token processLiteral() throws IOException {
		try {
			if (character == '\'') {
				errors.addErro("Erro Literal: Caractere ' \' ' nao e aceito para literal.", lexeme.toString(), line, column, "LEX");
			...
			lexeme.append(character);
			if (character != '\"') {
				errors.addErro("Erro Literal: Faltando fechar aspas com '\"'.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
	...
	private Token processId() throws IOException {
		try {
			...
			if (character == ';' || character == '(' || character == ')') {
				...
				lexeme.append(character);
				errors.addErro("Erro validacao Id: Nao e possivel gerar um id com a seguinte sequencia '" + lexeme.toString() + "'", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
		} catch (EOFException e) {
	...
	private Token processNumIntAndNumFloat() throws IOException {
		...
		try {
			...
			if (character == '.') {
				...
				if (!Character.isDigit(character)) {
					lexeme.append(character);
					errors.addErro("Erro Literal Numerico Decimal: Espera 'digito' depois do '.'", lexeme.toString(), line, column, "LEX");
					return this.nextToken();
				}
				while (Character.isDigit(character)) {
					lexeme.append(character);
			...
			if (character == 'E' || character == 'e') {
				...
				if (character != '-' && character != '+') {
					lexeme.append(character);
					errors.addErro("Erro Literal Numerico: Espera ['-', '+'] depois do ['E', 'e']", lexeme.toString(), line, column, "LEX");
					return this.nextToken();
				}
				lexeme.append(character);
				character = fLoader.getNextChar();
				if (!Character.isDigit(character)) {
					lexeme.append(character);
					errors.addErro("Erro Literal Numerico: Espera 'digito' depois do ['-', '+']", lexeme.toString(), line, column, "LEX");
					return this.nextToken();
				}
				while (Character.isDigit(character)) {
					lexeme.append(character);
			...
			if (character == ';'  || character == '(' || character == ')') {
				...
				lexeme.append(character);
				errors.addErro("Erro Literal Numerico: Caractere '" + character + "' nao e valido", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
		} catch (EOFException e) {
			try {
				...
				errors.addErro("Erro Literal Numerico: Nao e possivel gerar um numero com a seguinte sequencia '" + lexeme.toString() + "'", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
		}
	...
	private Token processAssign() throws IOException {
		try {
			...
			if (character != '-') {
				lexeme.append(character);
				errors.addErro("Erro Atribuicao: Espera '-' no lugar de '" + lexeme.toString() + "'.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
		} catch (Exception e) {
			if (lexeme.toString().length() != 2) {
				errors.addErro("Erro Atribuicao: Sequencia de caracteres nao formam uma atribuicao.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
		}
		lexeme.append(character);
	...
	private Token processRelop() throws IOException {
		try {
			...
			if (character != 'l' && character != 'g' && character != 'e' && character != 'd') {
				lexeme.append(character);
				errors.addErro("Erro Operador Logico: Espera [ 'l', 'g', 'e', 'd' ] no lugar de '" + lexeme.toString() + "'.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
			lexeme.append(character);
			character = fLoader.getNextChar();
			if (character != 't' && character != 'e' && character != 'q' && character != 'f') {
				lexeme.append(character);
				errors.addErro("Erro Operador Logico: Espera [ 't', 'e', 'q', 'f' ] no lugar de '" + lexeme.toString() + "'.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
			if ((character == 't' || character == 'e')
					&& (lexeme.charAt(lexeme.length() - 1) != 'l' && lexeme.charAt(lexeme.length() - 1) != 'g')) {
				lexeme.append(character);
				errors.addErro("Erro Operador Logico: Espera [ 'l', 'g' ] no lugar de '" + lexeme.toString() + "'.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			} else if (character == 'q' && lexeme.charAt(lexeme.length() - 1) != 'e') {
				lexeme.append(character);
				errors.addErro("Erro Operador Logico: Espera [ 'e' ] no lugar de '" + lexeme.toString() + "'.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			} else if (character == 'f' && lexeme.charAt(lexeme.length() - 1) != 'd') {
				lexeme.append(character);
				errors.addErro("Erro Operador Logico: Espera [ 'd' ] no lugar de '" + lexeme.toString() + "'.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
		} catch (EOFException e) {
			if (lexeme.toString().length() != 3) {
				errors.addErro("Erro Operador Logico: Caracteres informados nao forma um operador valido.", lexeme.toString(), line, column, "LEX");
				return this.nextToken();
			}
		}
		lexeme.append(character);
	...
	private void ignoreComments() throws IOException {
		try {
			...
			if (character != '#') {
				lexeme.append(character);
				errors.addErro("Erro Comentario: Espera {# comentario #}.", lexeme.toString(), line, column, "LEX");
				return;
			}
			do {
				lexeme.append(character);
